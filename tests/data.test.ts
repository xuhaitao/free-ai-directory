...
test("每日创收资讯与 Skill 热榜数量和来源可追溯",async()=>{const data=await loadOpportunitySnapshot();assert.ok(data.moneyNews.length>=40&&data.moneyNews.length<=60);assert.equal(data.skills.length,50);assert.ok(data.moneyNews.every(item=>item.discussionUrl.startsWith("https://news.ycombinator.com/")));assert.ok(data.skills.every(item=>item.url.startsWith("https://skills.sh/")));});
