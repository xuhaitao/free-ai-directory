...
import { loadWeeklyDigest } from "../src/weekly.ts";
...
test("AI 情报周报只聚合已有快照并保留公开来源",async()=>{const data=await loadWeeklyDigest();assert.equal(data.schemaVersion,3);assert.match(data.week,/^\d{4}-W\d{2}$/);assert.deepEqual([data.headlines.length,data.projects.length,data.models.length,data.moneyNews.length,data.skills.length,data.stockProjects.length,data.stockNews.length],[5,5,5,6,6,5,5]);assert.ok(Object.values(data.snapshotDays).every(days=>days>=1&&days<=7));assert.ok([data.headlines,data.projects,data.models,data.moneyNews,data.skills,data.stockProjects,data.stockNews].flat().every(item=>item.url.startsWith("https://")&&item.sourceLabels.length>0&&item.weeklyScore>=0&&item.weeklyScore<=100&&!Object.hasOwn(item,"sources")));assert.equal(data.changes.eligibleGroups.length+data.changes.skippedGroups.length,7);assert.equal(new Set([...data.changes.eligibleGroups,...data.changes.skippedGroups]).size,7);assert.ok([data.changes.persistent,data.changes.newcomers,data.changes.risers].flat().every(item=>item.url.startsWith("https://")&&item.sourceLabels.length>0&&item.latestRank>0));assert.ok(data.topics.length>=1&&data.topics.length<=5);assert.ok(data.topics.every(topic=>topic.itemCount>=3&&topic.sectionLabels.length>=2&&topic.items.length>=3&&topic.items.length<=8&&topic.items.every(item=>item.url.startsWith("https://")&&item.sourceLabels.length>0)));});
